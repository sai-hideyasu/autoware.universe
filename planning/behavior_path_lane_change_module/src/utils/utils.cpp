...
{
  Pose pose;
  pose.position = tier4_autoware_utils::createPoint(point.x(), point.y(), 0.0);
  ...
  const PredictedObject & object)
{
  ...
  const auto dist_to_base_link =
    motion_utils::calcSignedArcLength(path.points, current_ego_pose.position, obj_position);
  ...
  for (const auto & ego_edge_point : ego_polygon) {
    const auto ego_edge =
      tier4_autoware_utils::createPoint(ego_edge_point.x(), ego_edge_point.y(), 0.0);
    const auto dist_to_end = motion_utils::calcSignedArcLength(
  ...
  for (const auto & polygon_p : obj_polygon) {
    ...
    const auto dist_ego_to_obj =
      motion_utils::calcSignedArcLength(path.points, obj_p, path.points.back().point.pose.position);
  ...
  const PredictedObject & object)
{
  ...
  const auto & obj_position = object.kinematics.initial_pose_with_covariance.pose.position;
  const auto dist_to_base_link =
    motion_utils::calcSignedArcLength(path.points, obj_position, terminal_position);
  // we don't always have to check the distance accurately.
  ...
  for (const auto & polygon_p : obj_polygon) {
    ...
    const auto dist_obj_to_terminal =
      motion_utils::calcSignedArcLength(path.points, obj_p, terminal_position);
